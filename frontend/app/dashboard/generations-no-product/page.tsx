    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      {isLoading ? (
        ...
        <div className="bg-gray-50 rounded-lg p-12 text-center">
          <p className="text-gray-600 mb-4">No generations yet for &quot;No product&quot;</p>
          <Link href="/create" className="inline-block bg-vivid-yellow text-rich-black px-6 py-3 rounded-md font-semibold">
            Create image
          </Link>
        </div>
      ) : (
        <>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {items.map((gen: { id: string; output_image_url?: string; status: string; prompt?: string; final_prompt?: string; apply_brand_identity?: boolean; created_at: string }) => (
              <div key={gen.id} className="bg-white border border-gray-200 rounded-lg overflow-hidden">
                ...
                <div className="p-4">
                  ...
                  <div className="flex justify-between items-center text-xs text-gray-500">
                    ...
                    {gen.apply_brand_identity && <span className="text-vivid-yellow">Brand identity applied</span>}
                  </div>
                  {gen.output_image_url && (
                    <a
                      href={getAbsoluteImageUrl(gen.output_image_url) ?? gen.output_image_url}
                      download
                      className="mt-2 inline-block text-sm text-vivid-yellow hover:underline"
                    >
                      Download
                    </a>
                  )}
